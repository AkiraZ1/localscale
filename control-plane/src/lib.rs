...
pub mod google_oidc {
    ...
    impl std::fmt::Debug for ClientSecretRef {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            f.debug_tuple("ClientSecretRef")
                .field(&"[REDACTED]")
                .finish()
        }
    }
    ...
    impl GoogleOidcConfig {
        pub fn new(
            ...
            clock_skew: Duration,
            secret_ref: ClientSecretRef,
        ) -> Result<Self, OidcCoreError> {
            if issuer != "https://accounts.google.com" && issuer != "accounts.google.com" {
                return Err(OidcCoreError::InvalidConfiguration("issuer"));
            }
            if audience.is_empty()
                || scopes.iter().any(|s| s.is_empty())
                || !scopes.contains(&"openid")
            {
                return Err(OidcCoreError::InvalidConfiguration("audience/scopes"));
            }
            let https = redirect_uri.starts_with("https://");
            let loopback = redirect_uri.starts_with("http://127.0.0.1:")
                || redirect_uri.starts_with("http://localhost:");
            if (!https && !loopback)
                || redirect_uri.contains('?')
                || redirect_uri.contains('#')
                || redirect_uri.ends_with('/')
            {
                return Err(OidcCoreError::InvalidConfiguration("redirect_uri"));
            }
            if transaction_ttl.is_zero() || clock_skew > Duration::from_secs(300) {
                return Err(OidcCoreError::InvalidConfiguration("time"));
            }
            Ok(Self {
                issuer: issuer.into(),
        ...
        pub fn client_secret_ref(&self) -> &ClientSecretRef {
            &self.secret_ref
        }
        pub fn from_env(secret_ref: ClientSecretRef) -> Result<Self, OidcCoreError> {
            let issuer = std::env::var("LOCALSCALE_OIDC_ISSUER")
                .map_err(|_| OidcCoreError::InvalidConfiguration("issuer"))?;
            let audience = std::env::var("LOCALSCALE_OIDC_AUDIENCE")
                .or_else(|_| std::env::var("LOCALSCALE_GOOGLE_CLIENT_ID"))
                .map_err(|_| OidcCoreError::InvalidConfiguration("audience"))?;
            let redirect = std::env::var("LOCALSCALE_GOOGLE_REDIRECT_URI")
                .map_err(|_| OidcCoreError::InvalidConfiguration("redirect_uri"))?;
            let scopes = std::env::var("LOCALSCALE_OIDC_SCOPES")
        ...
        pub fn secret_reference_is_opaque(&self) -> bool {
            true
        }
    }
    ...
    #[derive(Debug, Default)]
    pub struct PkceTransactionStore {
        transactions: HashMap<String, PkceTransaction>,
    ...
    impl PkceTransactionStore {
        ...
        pub fn begin(
            ...
            return_path: &str,
            now: SystemTime,
        ) -> Result<PkceTransaction, OidcCoreError> {
            self.begin_with_ttl(config.transaction_ttl, return_path, now)
        }
        pub fn begin_with_ttl(
            &mut self,
            ttl: Duration,
            return_path: &str,
            now: SystemTime,
        ) -> Result<PkceTransaction, OidcCoreError> {
            if !valid_return_path(return_path) {
                return Err(OidcCoreError::InvalidReturnPath);
            }
            let (state, nonce, verifier) = (random(), random(), random());
            let tx = PkceTransaction {
                state: state.clone(),
                nonce,
            ...
            self.transactions.insert(state, tx.clone());
            Ok(tx)
        }
        pub fn consume(
            &mut self,
            ...
            if self.consumed.contains(state) {
                return Err(OidcCoreError::TransactionReplay);
            }
            let tx = self
                .transactions
                .remove(state)
                .ok_or(OidcCoreError::StateMismatch)?;
            if now >= tx.expires_at {
                self.consumed.insert(state.into());
                return Err(OidcCoreError::TransactionExpired);
            }
            self.consumed.insert(state.into());
            Ok(tx)
        }
    ...
    fn valid_return_path(p: &str) -> bool {
        p.starts_with('/')
            && !p.starts_with("//")
            && !p.contains('\\')
            && !p.contains("//")
    ...
    pub fn authorization_url(
        ...
        tx: &PkceTransaction,
    ) -> Result<String, OidcCoreError> {
        if !endpoints.authorization_endpoint.starts_with("https://") {
            return Err(OidcCoreError::InvalidEndpoint);
        }
        let mut q = form_urlencoded::Serializer::new(String::new());
        q.append_pair("client_id", &config.audience)
    ...
    impl<'a> Audience<'a> {
        fn contains(&self, expected: &str) -> bool {
            match self {
                ...
                Self::Many(a) => a.iter().any(|x| x == expected),
            }
        }
    }
    #[derive(Clone, PartialEq, Eq)]
    ...
    impl std::fmt::Debug for IdTokenClaims<'_> {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    pub struct IdTokenValidator<'a> {
        config: &'a GoogleOidcConfig,
    }
    impl<'a> IdTokenValidator<'a> {
        pub fn new(config: &'a GoogleOidcConfig) -> Self {
            Self { config }
        }
        pub fn validate(
            &self,
            ...
            now: i64,
            expected_nonce: &str,
        ) -> Result<ValidatedIdentity, OidcCoreError> {
            if c.issuer != "https://accounts.google.com" && c.issuer != "accounts.google.com" {
                return Err(OidcCoreError::InvalidIssuer);
            }
            if !c.audience.contains(&self.config.audience) {
                return Err(OidcCoreError::InvalidAudience);
            }
            if c.azp
                .is_some_and(|azp| azp != self.config.audience.as_str())
                || matches!(&c.audience, Audience::Many(audiences) if audiences.len() > 1 && c.azp.is_none())
            {
                return Err(OidcCoreError::InvalidAudience);
            }
            let skew = self.config.clock_skew.as_secs() as i64;
            if c.expires_at <= now - skew {
                return Err(OidcCoreError::Expired);
            }
            if c.issued_at > now + skew {
                return Err(OidcCoreError::IssuedInFuture);
            }
            if c.nonce != expected_nonce {
                return Err(OidcCoreError::NonceMismatch);
            }
            if c.subject.is_empty() {
                return Err(OidcCoreError::MissingSubject);
            }
            if !c.email_verified {
                return Err(OidcCoreError::UnverifiedEmail);
            }
    ...
    pub fn subject_key(identity: &ValidatedIdentity) -> String {
    ...
    impl AudienceWire {
        fn contains(&self, expected: &str) -> bool {
            match self {
                Self::One(v) => v == expected,
                Self::Many(v) => v.iter().any(|x| x == expected),
            }
        }
        fn requires_azp(&self) -> bool {
            matches!(self, Self::Many(v) if v.len() > 1)
        }
    }

    pub struct GoogleOidcProvider<T> {
        config: GoogleOidcConfig,
        transport: T,
        discovery: Option<GoogleDiscovery>,
        jwks: Option<CachedJwks>,
        cache_ttl: Duration,
        max_response_bytes: usize,
    }
    impl<T: HttpTransport> GoogleOidcProvider<T> {
        pub fn new(config: GoogleOidcConfig, transport: T) -> Self {
            Self {
                config,
                transport,
                discovery: None,
        ...
        pub fn with_cache_limits(mut self, ttl: Duration, max_response_bytes: usize) -> Self {
            self.cache_ttl = ttl;
        ...
        fn request(
            ...
            url: &str,
            body: String,
        ) -> Result<HttpResponse, OidcProviderError> {
            if !is_https(url) {
                return Err(OidcProviderError::InvalidDiscovery("endpoint"));
            }
            let r = self
                .transport
                .send(HttpRequest {
                ...
                .map_err(|e| match e {
                    TransportError::Timeout | TransportError::Network => {
                        OidcProviderError::Transport
                    }
                })?;
            if r.body.len() > self.max_response_bytes {
                return Err(OidcProviderError::ResponseTooLarge);
            }
            if !(200..300).contains(&r.status) {
                return Err(OidcProviderError::HttpStatus);
            }
            Ok(r)
        }
        pub fn discover(&mut self) -> Result<GoogleDiscovery, OidcProviderError> {
            ...
            let w: DiscoveryWire = serde_json::from_str(&r.body)
                .map_err(|_| OidcProviderError::InvalidDiscovery("json"))?;
            if w.issuer != "https://accounts.google.com" {
                return Err(OidcProviderError::InvalidDiscovery("issuer"));
            }
            if !endpoint_allowed(&w.authorization_endpoint, "accounts.google.com")
                || !endpoint_allowed(&w.token_endpoint, "oauth2.googleapis.com")
                || !endpoint_allowed(&w.jwks_uri, "www.googleapis.com")
            {
                return Err(OidcProviderError::InvalidDiscovery("endpoint"));
            }
            let d = GoogleDiscovery {
                issuer: w.issuer,
                authorization_endpoint: w.authorization_endpoint,
        ...
        fn load_jwks(&mut self, force: bool) -> Result<Vec<Jwk>, OidcProviderError> {
            let fresh = self.jwks.as_ref().is_some_and(|c| {
                !force
                    && SystemTime::now()
                        .duration_since(c.fetched)
                        .unwrap_or_default()
                        < self.cache_ttl
            });
            if fresh {
                return Ok(self.jwks.as_ref().unwrap().keys.clone());
            }
            let uri = self
                .discovery
                .as_ref()
            ...
            let w: JwksWire =
                serde_json::from_str(&r.body).map_err(|_| OidcProviderError::InvalidJwks)?;
            if w.keys.is_empty() || w.keys.len() > 100 {
                return Err(OidcProviderError::InvalidJwks);
            }
            self.jwks = Some(CachedJwks {
                keys: w.keys.clone(),
        ...
        pub fn exchange_code(
            ...
            tx: &PkceTransaction,
            code: &str,
        ) -> Result<TokenResponse, OidcProviderError> {
            self.exchange_code_with_redirect_uri(tx, code, &self.config.redirect_uri.clone())
        }
        pub fn exchange_code_with_redirect_uri(
            &mut self,
            tx: &PkceTransaction,
            code: &str,
            redirect_uri: &str,
            ...
            if redirect_uri != self.config.redirect_uri {
                return Err(OidcProviderError::RedirectMismatch);
            }
            let d = if let Some(d) = &self.discovery {
                d.clone()
            } else {
                self.discover()?
            };
            if code.is_empty() || tx.code_verifier().is_empty() {
                return Err(OidcProviderError::InvalidTokenResponse);
            }
            let mut q = form_urlencoded::Serializer::new(String::new());
            q.append_pair("code", code)
                .append_pair("client_id", &self.config.audience)
                .append_pair("client_secret", &self.config.secret_ref.0)
                .append_pair("redirect_uri", &self.config.redirect_uri)
                .append_pair("grant_type", "authorization_code")
                .append_pair("code_verifier", tx.code_verifier());
            let r = self.request("POST", &d.token_endpoint, q.finish())?;
            let w: TokenWire = serde_json::from_str(&r.body)
                .map_err(|_| OidcProviderError::InvalidTokenResponse)?;
            match (w.access_token, w.id_token, w.token_type) {
                (Some(a), Some(i), Some(t)) if !a.is_empty() && !i.is_empty() && t == "Bearer" => {
                    Ok(TokenResponse {
                        access_token: a,
                        id_token: i,
                        token_type: t,
                ...
                _ => Err(OidcProviderError::InvalidTokenResponse),
            }
        }
        pub fn validate_id_token(
            ...
            expected_nonce: &str,
            now: i64,
        ) -> Result<ValidatedIdentity, OidcProviderError> {
            let header =
                jsonwebtoken::decode_header(raw).map_err(|_| OidcProviderError::InvalidToken)?;
            if header.alg != jsonwebtoken::Algorithm::RS256 {
                return Err(OidcProviderError::UnsupportedAlgorithm);
            }
            ...
            if result.is_err() {
                keys = self.load_jwks(true)?;
                result = self.verify_with_key(raw, &kid, &keys, expected_nonce, now);
            }
            result
        }
        fn verify_with_key(
            ...
            let (n, e) = (
                key.n.as_deref().ok_or(OidcProviderError::InvalidJwks)?,
                key.e.as_deref().ok_or(OidcProviderError::InvalidJwks)?,
            );
            let decoding = jsonwebtoken::DecodingKey::from_rsa_components(n, e)
                .map_err(|_| OidcProviderError::InvalidJwks)?;
            let mut validation = jsonwebtoken::Validation::new(jsonwebtoken::Algorithm::RS256);
            validation.set_issuer(&["https://accounts.google.com"]);
            validation.set_audience(&[self.config.audience.as_str()]);
            validation.validate_exp = false;
            let data = jsonwebtoken::decode::<JwtClaims>(raw, &decoding, &validation)
                .map_err(|_| OidcProviderError::InvalidSignature)?;
            let c = data.claims;
            let skew = self.config.clock_skew.as_secs() as i64;
            if c.iss != "https://accounts.google.com"
                || !c.aud.contains(&self.config.audience)
                || c.azp
                    .as_deref()
                    .is_some_and(|azp| azp != self.config.audience.as_str())
                || (c.aud.requires_azp() && c.azp.is_none())
            {
            ...
            if c.nonce != expected_nonce || c.sub.is_empty() || !c.email_verified {
                return Err(OidcProviderError::InvalidToken);
            }
            Ok(ValidatedIdentity {
                issuer: "https://accounts.google.com".into(),
    ...
    fn is_https(s: &str) -> bool {
        s.starts_with("https://") && !s.contains(['?', '#', '@'])
    }
    fn endpoint_allowed(s: &str, host: &str) -> bool {
        is_https(s)
            && s.strip_prefix("https://")
                .is_some_and(|r| r.split('/').next() == Some(host))
    }
}
...
pub trait AuthProvider {
    ...
    fn exchange_code(
        &mut self,
        tx: &google_oidc::PkceTransaction,
        code: &str,
    ) -> Result<google_oidc::TokenResponse, AuthError>;
    fn validate_id_token(
        &mut self,
        raw: &str,
        nonce: &str,
        now: i64,
    ) -> Result<google_oidc::ValidatedIdentity, AuthError>;
...
impl<T: google_oidc::HttpTransport> AuthProvider for google_oidc::GoogleOidcProvider<T> {
    fn authorization_url(&self, tx: &google_oidc::PkceTransaction) -> Result<String, AuthError> {
        google_oidc::authorization_url(self.config(), &google_oidc::OidcEndpoints::google(), tx)
    ...
    fn validate_id_token(
        ...
        self.validate_id_token(raw, nonce, now)
            .map_err(|_| AuthError::Provider)
    }
    fn transaction_ttl(&self) -> std::time::Duration {
...
pub struct AuthService<P> {
    provider: P,
    transactions: google_oidc::PkceTransactionStore,
...
impl<P: AuthProvider> AuthService<P> {
    pub fn new(provider: P, session_ttl: std::time::Duration, _redirect_uri: &str) -> Self {
        Self {
            provider,
            transactions: google_oidc::PkceTransactionStore::new(),
    ...
    pub fn handle(&mut self, request: AuthRequest) -> AuthResponse {
        self.handle_at(request, std::time::SystemTime::now())
    }
    pub fn handle_at(&mut self, request: AuthRequest, now: std::time::SystemTime) -> AuthResponse {
        self.sessions
            .retain(|_, session| session.expires_at > now && !session.revoked);
    ...
    fn start(&mut self, request: &AuthRequest, now: std::time::SystemTime) -> AuthResponse {
        let callback = request
            .query
            .get("app_callback")
            .filter(|u| valid_loopback_callback(u))
            .cloned();
        if request.query.contains_key("app_callback") && callback.is_none() {
            return AuthResponse::new(400);
        }
        let flutter_state = request
            .query
            .get("state")
            .filter(|state| !state.is_empty())
            .cloned();
        if callback.is_some() && flutter_state.is_none() {
            return AuthResponse::new(400);
        }
        let tx = match self
            .transactions
            .begin_with_ttl(self.provider.transaction_ttl(), "/", now)
        {
            Ok(t) => t,
            Err(_) => return AuthResponse::new(500),
        };
        if let (Some(callback), Some(flutter_state)) = (callback, flutter_state) {
        ...
        match self.provider.authorization_url(&tx) {
            Ok(url) => AuthResponse {
                status: 302,
                headers: vec![("Location".into(), url)],
                body: String::new(),
            },
            Err(_) => AuthResponse::new(502),
        }
    }
    fn callback(&mut self, request: AuthRequest, now: std::time::SystemTime) -> AuthResponse {
        ...
        let (Some(code), Some(state)) = (request.query.get("code"), request.query.get("state"))
        else {
            return AuthResponse::new(400);
        };
        ...
        let tx = match self.transactions.consume(state, now) {
            Ok(t) => t,
            Err(_) => return AuthResponse::new(400),
        };
        let pending_callback = self
            .pending_callbacks
            .remove(state)
            .filter(|pending| valid_loopback_callback(&pending.callback));
        ...
        let identity = match self
            .provider
            .validate_id_token(&token.id_token, tx.nonce(), now_secs)
        {
            Ok(i) => i,
        ...
        let location = if let Some(pending) = location {
            ...
            self.handoffs.insert(
                ...
                Handoff {
                    ...
                    flutter_state: flutter_state.clone(),
                    expires_at: now + self.session_ttl,
                },
            );
            let separator = if callback.contains('?') { '&' } else { '?' };
            let encoded_handoff =
                form_urlencoded::byte_serialize(handoff.as_bytes()).collect::<String>();
            let encoded_state =
                form_urlencoded::byte_serialize(flutter_state.as_bytes()).collect::<String>();
            format!("{callback}{separator}handoff={encoded_handoff}&state={encoded_state}")
        } else {
            "/".to_owned()
        };
        let headers = if has_handoff {
            Vec::new()
        } else {
            vec![(
                "Set-Cookie".into(),
                format!(
                    "localscale_session={}; Path=/; Max-Age={}; HttpOnly; Secure; SameSite=Strict",
                    sid,
        ...
        AuthResponse {
            ...
            headers: {
                let mut h = headers;
                h.insert(0, ("Location".into(), location));
                h
            },
            body: String::new(),
        }
    ...
    fn provider_failure(pending: Option<PendingCallback>, status: u16) -> AuthResponse {
        let Some(pending) = pending else {
            return AuthResponse::new(status);
        };
        let separator = if pending.callback.contains('?') {
            '&'
        } else {
            '?'
        };
        let encoded_state =
            form_urlencoded::byte_serialize(pending.flutter_state.as_bytes()).collect::<String>();
        AuthResponse {
            status: 303,
            headers: vec![(
                "Location".into(),
                format!(
                    "{}{separator}error=authentication_failed&state={encoded_state}",
                    pending.callback
                ),
            )],
            body: String::new(),
    ...
    fn bridge(&mut self, request: &AuthRequest, now: std::time::SystemTime) -> AuthResponse {
        let (Some(token), Some(callback), Some(flutter_state)) = (
            request.query.get("handoff"),
            request.query.get("callback"),
            request.query.get("state"),
        ) else {
            return AuthResponse::new(400);
        };
        let Some(handoff) = self.handoffs.get(token).cloned() else {
            return AuthResponse::new(401);
        };
        if handoff.expires_at <= now
            || handoff.callback != *callback
            || handoff.flutter_state != *flutter_state
            || !valid_loopback_callback(callback)
        {
            return AuthResponse::new(401);
        }
        ...
        let Some(session) = self
            .sessions
            .get(&sid)
            .filter(|s| s.expires_at > now && !s.revoked)
        else {
            return AuthResponse::new(401);
        };
        let expires = session
        ...
        AuthResponse {
            ...
            body: serde_json::json!({ "session_id": sid, "expires_at": expires }).to_string(),
        }
    }
    fn session(&mut self, request: &AuthRequest, now: std::time::SystemTime) -> AuthResponse {
        match self.session_id(request).and_then(|id| self.sessions.get(&id)).filter(|s| !s.revoked && s.expires_at > now) { Some(s) => AuthResponse { status: 200, headers: vec![("Content-Type".into(), "application/json".into())], body: serde_json::json!({ "authenticated": true, "issuer": s.issuer, "subject": s.subject }).to_string() }, None => AuthResponse::new(401) }
    }
    fn logout(&mut self, request: &AuthRequest, now: std::time::SystemTime) -> AuthResponse {
        let Some(id) = self.session_id(request) else {
            return AuthResponse::new(401);
        };
        let Some(csrf) = request.headers.get("X-CSRF-Token") else {
            return AuthResponse::new(403);
        };
        if !constant_time_eq(&id, csrf) {
            return AuthResponse::new(403);
        }
        if let Some(s) = self.sessions.get_mut(&id) {
            ...
            return AuthResponse {
                ...
                headers: vec![(
                    "Set-Cookie".into(),
                    "localscale_session=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Strict"
                        .into(),
                )],
                ..Default::default()
            };
        }
        AuthResponse::new(401)
    }
    fn session_id(&self, request: &AuthRequest) -> Option<String> {
        request
            .headers
            .get("Cookie")?
            .split(';')
            .map(str::trim)
            .find_map(|v| v.strip_prefix("localscale_session="))
            .filter(|v| {
                !v.is_empty()
                    && v.bytes()
...
fn constant_time_eq(a: &str, b: &str) -> bool {
    let aa = a.as_bytes();
    let bb = b.as_bytes();
    let mut diff = (aa.len() ^ bb.len()) as u8;
...
mod phase3_red_tests {
    ...
    fn service() -> AuthService<Provider> {
        AuthService::new(
            Provider,
            Duration::from_secs(60),
    ...
    fn assert_provider_failure_redirects(fail_exchange: bool, fail_validation: bool) {
        ...
        let start = service.handle_at(
            AuthRequest::get("/oauth/google/start")
                .query("app_callback", callback)
                .query("state", flutter_state),
            now(),
        );
        ...
        let response = service.handle_at(
            AuthRequest::get("/oauth/google/callback")
                .query("code", sensitive_code)
                .query("state", &server_state),
            now(),
        );
        assert_eq!(response.status, 303);
        assert_eq!(
            response.header("Location").unwrap(),
            format!("{callback}?error=authentication_failed&state={flutter_state}")
        );
        ...
        assert!(
            !rendered.contains(sensitive_code)
                && !rendered.contains("access-secret")
                && !rendered.contains("id-secret")
        );
        assert_eq!(
            service
                .handle_at(
    ...
    fn session_json_escapes_authenticated_identity_fields() {
        ...
        let response = service.handle_at(
            AuthRequest::get("/auth/session")
                .header("Cookie", &format!("localscale_session={sid}")),
            now(),
        );
    ...
    fn start_is_a_safe_302_and_stores_pkce_state() {
        ...
        assert!(response
            .header("Location")
            .unwrap()
            .starts_with("https://accounts.google.com/auth?state="));
    }
    ...
    fn app_handoff_requires_flutter_state() {
        ...
        assert_eq!(
            service
                .handle_at(
                    AuthRequest::get("/oauth/google/start")
                        .query("app_callback", "http://127.0.0.1:43126/oauth/callback"),
                    now()
                )
                .status,
            400
    ...
    fn callback_consumes_state_and_sets_opaque_secure_cookie() {
        ...
        let state = start
            ...
            .nth(1)
            .unwrap();
        let callback = AuthRequest::get("/oauth/google/callback")
        ...
        assert!(
            !cookie.contains("provider-code")
                && !cookie.contains("access-secret")
                && !cookie.contains("id-secret")
        );
    }
    ...
    fn logout_requires_csrf_and_revokes_session() {
        ...
        assert_eq!(
            service
                .handle_at(
                    AuthRequest::get("/auth/session").header("Cookie", &cookie),
                    now()
                )
                .status,
            200
        ...
        assert_eq!(
            service
                .handle_at(
                    AuthRequest::post("/auth/logout").header("Cookie", &cookie),
                    now()
                )
                .status,
            403
        ...
        let out = service.handle_at(
            AuthRequest::post("/auth/logout")
                .header("Cookie", &cookie)
                .header("X-CSRF-Token", sid),
            now(),
        );
    ...
    fn callback_rejects_denial_mismatch_replay_and_expired_state() {
        ...
        assert_eq!(
            service
                .handle_at(
                    AuthRequest::get("/oauth/google/callback").query("error", "access_denied"),
                    now()
                )
                .status,
            400
    ...
    fn dynamic_loopback_handoff_contract_is_bound_and_token_free() {
        ...
        assert!(
            !location.contains("provider-code")
                && !location.contains("access-secret")
                && !location.contains("id-secret")
        );
        ...
        let exchange = AuthRequest::get("/auth/session/bridge")
        ...
        assert_eq!(
            service
                .handle_at(
                    AuthRequest::get("/auth/session/bridge")
                        .query("handoff", &handoff)
                        .query("callback", callback)
                        .query("state", flutter_state),
                    now()
        ...
        assert_eq!(
            service
                .handle_at(
                    AuthRequest::get("/auth/session/bridge")
                        .query("handoff", handoff)
                        .query("callback", "http://127.0.0.1:43125/oauth/callback")
                        .query("state", flutter_state),
                    now()
                )
                .status,
            ...
            service
                .handle_at(
                    AuthRequest::get("/auth/session/bridge")
                        .query("handoff", handoff)
                        .query("callback", callback)
                        .query("state", "wrong-flutter-state"),
                    now()
                )
                .status,
            401
...
mod tests {
    ...
    fn transaction_is_random_pkce_and_single_use() {
        ...
        assert!(matches!(
            store.consume(tx.state(), now),
            Err(OidcCoreError::TransactionReplay)
        ));
    }
    ...
    fn callback_rejects_mismatch_and_expiry() {
        ...
        assert!(matches!(
            store.consume("wrong", now),
            Err(OidcCoreError::StateMismatch)
        ));
        assert!(matches!(
            store.consume(tx.state(), now + Duration::from_secs(301)),
            Err(OidcCoreError::TransactionExpired)
        ));
    }
    ...
    fn authorization_url_contains_safe_parameters() {
        ...
        let mut store = PkceTransactionStore::new();
        let tx = store.begin(&config(), "/", now).unwrap();
        let url = authorization_url(&config(), &OidcEndpoints::google(), &tx)
            .unwrap()
            .to_string();
    ...
    fn claims() -> IdTokenClaims<'static> {
        IdTokenClaims {
            issuer: "https://accounts.google.com",
            audience: Audience::Many(vec!["client-id".into()]),
            azp: None,
    ...
    fn claims_validate_google_issuer_audience_times_nonce_email_and_subject() {
        let cfg = config();
        let v = IdTokenValidator::new(&cfg);
        let valid = claims();
        assert_eq!(
            v.validate(&valid, 1_700_000_010, "n").unwrap().subject,
            "stable-sub"
        );
        let bads = [
            IdTokenClaims {
            ...
            IdTokenClaims {
                audience: Audience::One("other"),
                ..valid.clone()
            },
            IdTokenClaims {
                issued_at: 17_000_001_000,
                ..valid.clone()
            },
            IdTokenClaims {
        ...
        for bad in bads {
            assert!(v.validate(&bad, 1_700_000_010, "n").is_err());
        }
    }
    ...
    fn claims_with_multiple_audiences_require_matching_azp() {
        let cfg = config();
        let v = IdTokenValidator::new(&cfg);
        let valid = IdTokenClaims {
            audience: Audience::Many(vec!["client-id".into(), "other-client".into()]),
            azp: Some("client-id"),
            ..claims()
        };
        assert!(v.validate(&valid, 1_700_000_010, "n").is_ok());
        let missing = IdTokenClaims {
            azp: None,
            ..valid.clone()
        };
        assert!(matches!(
            v.validate(&missing, 1_700_000_010, "n"),
            Err(OidcCoreError::InvalidAudience)
        ));
        let mismatched = IdTokenClaims {
            azp: Some("other-client"),
            ..valid
        };
        assert!(matches!(
            v.validate(&mismatched, 1_700_000_010, "n"),
            Err(OidcCoreError::InvalidAudience)
        ));
        let single_mismatched = IdTokenClaims {
            audience: Audience::One("client-id"),
            azp: Some("other-client"),
            ..claims()
        };
        assert!(matches!(
            v.validate(&single_mismatched, 1_700_000_010, "n"),
            Err(OidcCoreError::InvalidAudience)
        ));
        let single_matching = IdTokenClaims {
            audience: Audience::One("client-id"),
            azp: Some("client-id"),
            ..claims()
        };
    ...
    fn phase2_provider_rejects_untrusted_discovery_endpoints() {
        let transport = TestTransport::new(vec![HttpResponse::ok(
            r#"{"issuer":"https://accounts.google.com","authorization_endpoint":"http://evil.test/auth","token_endpoint":"https://oauth2.googleapis.com/token","jwks_uri":"https://www.googleapis.com/oauth2/v3/certs"}"#,
        )]);
        let mut provider = GoogleOidcProvider::new(config(), transport);
        assert!(matches!(
            provider.discover(),
            Err(OidcProviderError::InvalidDiscovery(_))
        ));
    }
    ...
    fn phase2_rejects_redirect_uri_mismatch_before_network() {
        ...
        assert_eq!(
            provider.exchange_code_with_redirect_uri(&tx, "code", "https://evil.test/cb"),
            Err(OidcProviderError::RedirectMismatch)
        );
    }
    ...
    fn phase2_rejects_invalid_signature_and_missing_claim_token() {
        ...
        let mut provider = GoogleOidcProvider::new(
            config(),
            TestTransport::new(vec![HttpResponse::ok(discovery), HttpResponse::ok(jwks)]),
        );
    ...
    fn phase2_provider_posts_exact_pkce_exchange_without_secret_exposure() {
        let transport = TestTransport::new(vec![
            HttpResponse::ok(
                r#"{"issuer":"https://accounts.google.com","authorization_endpoint":"https://accounts.google.com/auth","token_endpoint":"https://oauth2.googleapis.com/token","jwks_uri":"https://www.googleapis.com/oauth2/v3/certs"}"#,
            ),
