...
void main() {
  test('parses a running Host response and preserves Onion endpoint', () {
    final status = ServiceStatus.fromJson({
      'mode': 'host', 'state': 'running', 'onion_endpoint': 'abc123.onion',
    });
  ...
  test('rejects unknown agent mode', () {
    expect(() => ServiceStatus.fromJson({'mode': 'invalid', 'state': 'stopped'}), throwsFormatException);
  });
  ...
  test('client maps typed operations to local agent paths', () async {
    ...
    expect((await client.setMode(LocalScaleMode.host)).mode, LocalScaleMode.host);
