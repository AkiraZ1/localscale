...
void main() {
  test('start URL contains authorization code + PKCE parameters', () async {
    ...
    await expectLater(
      client(
              launcher: launcher,
              callback: Uri.parse(
                  'http://127.0.0.1/callback?code=c&state=expected-state'))
          .authorize(),
      completes,
    );
    ...
    expect(launcher.launched!.queryParameters,
        containsPair('redirect_uri', 'http://127.0.0.1/callback'));
    expect(launcher.launched!.queryParameters,
  ...
  test('callback success exchanges code without exposing a token', () async {
    ...
    await client(
            callback: Uri.parse(
                'http://127.0.0.1/callback?code=abc&state=expected-state'),
            exchanger: exchanger)
        .authorize();
  ...
  test('state mismatch maps to stateMismatch', () async {
    await expectLater(
        client(
                callback:
                    Uri.parse('http://127.0.0.1/callback?code=abc&state=wrong'))
            .authorize(),
        throwsA(isA<AuthException>()
