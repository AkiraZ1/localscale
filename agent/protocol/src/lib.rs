...
pub struct HostInvitation {
    ...
    invitation_secret: zeroize::Zeroizing<String>,
}
impl HostInvitation {
    pub fn new(
        host_node_id: &str,
        ...
        if !valid_node_id(host_node_id) {
            return Err(ValidationError::InvalidNodeId);
        }
        if !valid_onion_hostname(public_endpoint) || !valid_invitation_secret(invitation_secret) {
            return Err(ValidationError::InvalidNonce);
        }
        Ok(Self {
            host_node_id: host_node_id.into(),
            public_endpoint: public_endpoint.into(),
            invitation_secret: zeroize::Zeroizing::new(invitation_secret.into()),
        })
    }
    pub fn host_node_id(&self) -> &str {
        &self.host_node_id
    }
    ...
    pub(crate) fn secret(&self) -> &str {
        self.invitation_secret.as_str()
    }
    pub fn diagnostics(&self) -> String {
        format!(
            "configured host={} endpoint={} transport=unavailable",
            self.host_node_id, self.public_endpoint
        )
    }
}
...
impl ClientConfig {
    pub fn from_invitation(
        ...
        Self::client(
            client_node_id,
            invitation.host_node_id(),
            invitation.public_endpoint(),
            invitation.secret(),
        )
    }
    pub fn client(
        ...
        if !valid_node_id(client_node_id) || !valid_node_id(host_node_id) {
            return Err(ValidationError::InvalidNodeId);
        }
        if !valid_onion_hostname(public_endpoint) || !valid_invitation_secret(invitation_secret) {
            return Err(ValidationError::InvalidNonce);
    ...
    pub fn diagnostics(&self) -> String {
        format!(
            "configured client={} host={} endpoint={} transport=unavailable",
            self.client_node_id, self.host_node_id, self.public_endpoint
        )
    }
}
...
impl PeerConfig {
    pub fn client(
        ...
        public_endpoint: &str,
        invitation_secret: &str,
    ) -> Result<ClientConfig, ValidationError> {
        ClientConfig::client(
            client_node_id,
            host_node_id,
            public_endpoint,
...
fn validate_syntax(message: &Message) -> Result<(), ParseError> {
    let node = match message {
        Message::HandshakeInit { node_id, .. }
        | Message::HandshakeResponse { node_id, .. }
        | Message::Keepalive { node_id, .. }
        | Message::MtuProbe { node_id, .. } => node_id,
    };
    if !valid_node_id(node) {
        return Err(ParseError::InvalidNodeId);
    }
    match message {
        Message::HandshakeInit { nonce, .. } | Message::HandshakeResponse { nonce, .. }
            if !valid_nonce(nonce) =>
        {
            Err(ParseError::InvalidNonce)
        }
        Message::MtuProbe { mtu, .. } if *mtu < MIN_MTU || *mtu > MAX_MTU => {
            Err(ParseError::InvalidMtu)
        }
        _ => Ok(()),
    }
}
...
fn check_window(timestamp: u64, now: u64) -> Result<(), ValidationError> {
    if timestamp.abs_diff(now) > REPLAY_WINDOW_SECS {
        Err(ValidationError::TimestampOutsideReplayWindow)
    } else {
        Ok(())
    }
}
...
impl ReplayGuard {
    pub fn accept(
        ...
        self.seen
            .retain(|_, timestamp| now.saturating_sub(*timestamp) <= REPLAY_WINDOW_SECS);
    ...
    pub fn accept_session(
        ...
        if !valid_node_id(peer_node_id) {
            return Err(ValidationError::InvalidNodeId);
        }
        if let Some((existing_peer, existing_role)) = self.sessions.get(nonce) {
            if existing_peer != peer_node_id || *existing_role != role {
                return Err(ValidationError::WrongRole);
            }
            return Err(ValidationError::ReplayDetected);
        }
        self.sessions
            .insert(*nonce, (peer_node_id.to_string(), role));
        Ok(())
    }
}
...
impl CryptoKey {
    ...
    fn record_nonce(&self, nonce: [u8; NONCE_LEN]) -> Result<(), CryptoError> {
        ...
        if !tracked.insert(nonce) {
            return Err(CryptoError::NonceReuse);
        }
        Ok(())
    }
    fn cipher(&self) -> XChaCha20Poly1305 {
        XChaCha20Poly1305::new(Key::from_slice(&self.secret[..]))
    }
}
...
impl HandshakeEnvelope {
    pub fn seal(
        ...
        if !key.sealing_allowed {
            return Err(CryptoError::UnsafeNoncePolicy);
        }
    ...
    pub fn seal_with_allocator<A: NonceAllocator + ?Sized>(
        key: &CryptoKey,
        role: Role,
        node_id: &str,
        allocator: &mut A,
    ...
    fn seal_with_nonce_internal(
        ...
        if enforce_policy && !key.sealing_allowed {
            return Err(CryptoError::UnsafeNoncePolicy);
        }
        if !valid_node_id(node_id) {
            return Err(CryptoError::InvalidNodeId);
        }
        if !enforce_policy {
            return Self::seal_unchecked(key, role, node_id, nonce, plaintext);
        }
        key.record_nonce(nonce)?;
        let ad = associated_data(role, node_id);
        let ciphertext = match key.cipher().encrypt(
        ...
        let mut bytes =
            Vec::with_capacity(4 + 1 + 2 + node.len() + NONCE_LEN + 4 + ciphertext.len());
    ...
    pub fn open(
        ...
        if !valid_node_id(expected_node_id) {
            return Err(CryptoError::AuthenticationFailed);
        }
        let parsed = parse_envelope(&self.bytes)?;
        if parsed.role != expected_role || parsed.node_id != expected_node_id {
            return Err(CryptoError::AuthenticationFailed);
        }
...
impl CryptoProvider {
    ...
    pub fn load_key(bytes: &[u8]) -> Result<CryptoKey, CryptoError> {
...
fn parse_envelope(bytes: &[u8]) -> Result<ParsedEnvelope, CryptoError> {
    if bytes.len() < 4 + 1 + 2 + NONCE_LEN + 4 || &bytes[..4] != MAGIC {
        return Err(CryptoError::MalformedEnvelope);
    }
    let role = match bytes[4] {
        0 => Role::Host,
        1 => Role::Cliente,
    ...
    let nonce_start = node_start
        .checked_add(node_len)
        .ok_or(CryptoError::MalformedEnvelope)?;
    let len_start = nonce_start
        .checked_add(NONCE_LEN)
        .ok_or(CryptoError::MalformedEnvelope)?;
    let end_header = len_start
        .checked_add(4)
        .ok_or(CryptoError::MalformedEnvelope)?;
    if end_header > bytes.len() {
        return Err(CryptoError::MalformedEnvelope);
    }
    let node_id = std::str::from_utf8(&bytes[node_start..nonce_start])
    ...
    let mut nonce = [0u8; NONCE_LEN];
    nonce.copy_from_slice(&bytes[nonce_start..len_start]);
    let ciphertext_len =
        u32::from_be_bytes(bytes[len_start..end_header].try_into().unwrap()) as usize;
    let end = end_header
        .checked_add(ciphertext_len)
        .ok_or(CryptoError::MalformedEnvelope)?;
    if ciphertext_len < 16 || end != bytes.len() {
        return Err(CryptoError::MalformedEnvelope);
    }
    Ok(ParsedEnvelope {
        role,
        node_id,
        nonce,
        ciphertext: bytes[end_header..end].to_vec(),
    })
}
