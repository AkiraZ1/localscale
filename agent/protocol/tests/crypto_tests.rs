use localscale_agent_protocol::{
    CryptoError, CryptoKey, HandshakeEnvelope, NonceAllocator, Role, VERSION,
};
...
fn tampering_is_an_authentication_failure() {
    ...
    assert!(matches!(
        envelope.open(&key, Role::Host, "host-1"),
...
fn wrong_role_and_node_are_authentication_failures() {
    ...
    let envelope = HandshakeEnvelope::seal(&key, Role::Host, "host-1", b"hello").unwrap();
    assert!(matches!(
        envelope.open(&key, Role::Cliente, "host-1"),
        Err(CryptoError::AuthenticationFailed)
    ));
    assert!(matches!(
        envelope.open(&key, Role::Host, "other"),
        Err(CryptoError::AuthenticationFailed)
    ));
}
...
fn nonce_reuse_is_rejected() {
    ...
    assert!(matches!(
        HandshakeEnvelope::seal_with_nonce(&key, Role::Host, "host-1", nonce, b"two"),
...
fn raw_key_reload_rejects_unsafe_sealing_after_restart() {
    ...
    assert!(
        HandshakeEnvelope::seal_with_nonce(&first, Role::Host, "host-1", nonce, b"one").is_err()
    );
    let reloaded = CryptoKey::from_bytes(&bytes).unwrap();
    assert!(matches!(
        HandshakeEnvelope::seal(&reloaded, Role::Host, "host-1", b"two"),
        Err(CryptoError::UnsafeNoncePolicy)
    ));
}

struct DurableAllocator {
    next: u8,
}
...
fn caller_owned_durable_allocator_allows_raw_key_roundtrip_without_reuse() {
    ...
    let first =
        HandshakeEnvelope::seal_with_allocator(&key, Role::Host, "host-1", &mut allocator, b"one")
            .unwrap();
    let reloaded = CryptoKey::from_bytes(&[7u8; 32]).unwrap();
    let second = HandshakeEnvelope::seal_with_allocator(
        &reloaded,
        Role::Host,
        "host-1",
        &mut allocator,
...
fn malformed_envelopes_are_rejected() {
    assert!(matches!(
        HandshakeEnvelope::from_bytes(b"LS"),
    ...
    bytes[0] = b'X';
    assert!(matches!(
        HandshakeEnvelope::from_bytes(&bytes),
        Err(CryptoError::MalformedEnvelope)
    ));
}
...
fn wrong_key_cannot_open_envelope() {
    ...
    assert!(matches!(
        envelope.open(
            &CryptoKey::from_bytes(&[8u8; 32]).unwrap(),
            Role::Host,
            "host-1"
        ),
        Err(CryptoError::AuthenticationFailed)
